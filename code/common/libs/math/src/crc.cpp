#include "math/crc.h"

namespace math {

/**
 * @brief Lookup table for CRC-8-CCITT (also known as CRC-8-ATM)
 *
 * Polynomial:    0x07 (x^8 + x^2 + x + 1)
 * Initial value: 0x00
 * Input reflected:  false
 * Output reflected: false
 * Final XOR value:  0x00
 *
 * This table allows efficient CRC computation by replacing bit-wise
 * operations with a simple table lookup and XOR.
 */
constexpr uint8_t crc8_ccitt_table[256] = {
    };

uint8_t generateCrc8(std::span<uint8_t> data) {
    uint8_t crc = 0x00;
    for (const uint8_t byte : data) {
        crc = crc8_ccitt_table[crc ^ byte];
    }
    return crc;
}

}  // namespace math
